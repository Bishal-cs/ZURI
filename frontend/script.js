/* ================================================================
   S.Y.L.P.H. Frontend — Main Application Logic
   ================================================================

   ARCHITECTURE OVERVIEW
   ---------------------
   This file powers the entire frontend of the S.Y.L.P.H. AI assistant.
   It handles:
   ...
   4. ANIMATED ORB — A WebGL-powered visual orb (rendered by a
      ...
      "activates" when S.Y.L.P.H. is speaking and goes idle otherwise.
 ...
 * (e.g., "https://Sylph.example.com"). During local development, it falls
...
/*
 * currentMode — Which AI pipeline to use: 'Sylph', 'general', or 'realtime'.
 * - Sylph:   Unified route; brain classifies, then routes to general or realtime.
 * - general:  Direct /chat/stream (no web search).
 * - realtime: Direct /chat/realtime/stream (with Tavily web search).
 */
let currentMode = 'Sylph';
 ...
 */
const SETTINGS_KEY = 'Sylph_settings';
...
const btnSylph    = $('btn-Sylph');      // The "Sylph" mode button (unified, brain-routed)
...
const activityPanel       = $('activity-panel');          // Left panel for Sylph activity flow
...
function init() {
    if (!chatMessages || !messageInput) {
        console.error('[Sylph] Required DOM elements (chat-messages, message-input) not found.');
        return;
    }
    loadSettings();
...
function bindEvents() {
    ...
    // MODE TOGGLE BUTTONS — Switch between Sylph, General, and Realtime modes
    if (btnSylph)   btnSylph.addEventListener('click',   () => setMode('Sylph'));
...
function setMode(mode) {
    currentMode = mode;
    if (btnSylph)   btnSylph.classList.toggle('active', mode === 'Sylph');
...
/* ================================================================
   ...
   consists of:
     - An avatar circle ("J" for Sylph, "U" for user)
 ...
 *     <div class="msg-body">
 *       <div class="msg-label">Sylph (General) | You</div>
...
function addMessage(role, text) {
    ...
    label.textContent = role === 'assistant'
        ? `Sylph  (${currentMode === 'Sylph' ? 'Sylph' : currentMode === 'realtime' ? 'Realtime' : 'General'})`
        : 'You';
...
function addTypingIndicator() {
    ...
    const label = document.createElement('div');
    label.className = 'msg-label';
    label.textContent = `Sylph  (${currentMode === 'Sylph' ? 'Sylph' : currentMode === 'realtime' ? 'Realtime' : 'General'})`;
...
async function sendMessage(textOverride) {
    ...
    const endpoint = currentMode === 'Sylph'
        ? '/chat/Sylph/stream'
        ...
        while (!streamDone) {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    // ACTIVITY — Sylph flow (query detected, decision, routing): show in left panel
                    if (data.activity) {
