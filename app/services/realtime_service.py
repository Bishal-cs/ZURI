...
class RealtimeGroqService(GroqService):
    ...
    def search_tavily(self, query:str, num_results: int = 7) -> Tuple[str, Optional[dict]]:
        ...
        try:
            ...
            logger.error("Error performing Tavily serach: %s",e)
            return ("", None)
            ...
            if formatted_results:
                ...
                yield {
                    "_activity": {
                        ...
                        "message": f"Search completed: {num_results} results, {len(formatted_results)} chars of content"
                    }
                }
            else:
                logger.warning("[REALTIME] Tavily returned no results for: %s", search_query)
    ...
    def stream_response_with_prefetched(
        ...
        try:
            ...
            logger.error(
                "Error in realtime stream_response_with_prefetched: %s",
                e,
                exc_info=True
            )
            raise