...
logger = logging.getLogger("SYLPH")
...
class RealtimeGroqService(GroqService):
    ...
    def _extract_search_query(
        ...
        try:
            ...
            response = self._fast_11m.invoke(full_prompt)
            ...
            logger.warning(
                "Query extraction failed (%s), using raw question", e
            )
            return question
    ...
    def get_response(
        self, question: str, chat_history: Optional[List[tuple]] = None, key_start_index: int = 0
    ) -> str:
        try:
            ...
            t0 = time.perf_counter()
            response_content = self._invoke_11m(prompt, messages, question, key_start_index=key_start_index)
    ...
    def prefetch_web_search(
        ...
        try:
            ...
            logger.warning("Pre-fetch failed: %s", e)
            return ("", None)
    ...
    def stream_response(
        ...
        try:
            ...
            yield from self._stream_11m(prompt, messages, question, key_start_index=key_start_index)
