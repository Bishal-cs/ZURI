...
_PRIMARY_BRAIN_PROMPT = """You are the decision-maker for SYLPH. Classify the user's message into EXACTLY ONE category.
...
- Example: User said "open sylph4everyone.com", assistant opened wrong site, user says "it's integer 4 not f-o-r" → STILL a task
...
_TASK_BRAIN_PROMPT = """You are a very accurate Decision-Making Model for SYLPH. You decide what kind of task the user wants and extract the CLEAN query/topic for each task.
...
-> 'open (website/app name)' — Open a website or app.
   ...
   "Open sylph4everyone.com" → open sylph4everyone.com
...
-> 'play (song/video name)' — Play music or video on YouTube.
   ...
   "Hello sylph can you play Shape of You on YouTube" → play Shape of You
   "Hey sylph Teja song Dhurandhar title track can you play that on YouTube" → play Teja Dhurandhar title track
...
-> 'google_search (search topic)' — Search something on Google.
   ...
   "Hey sylph look up best restaurants nearby" → google_search best restaurants nearby
...
- "it's integer 4 not f-o-r" (after "open sylphforeveryone.com") → open sylph4everyone.com
...
=== CRITICAL RULES ===
*** Extract ONLY the relevant topic/query — REMOVE greetings (hello, hey, hi), assistant name (sylph), filler words (can you, please, for me), platform names (on YouTube, on Google), and command words from the query. ***
...
class BrainService:
    ...
    _TASK_FEW_SHOTS = [
        ...
        ("hello sylph can you play Shape of You", "play Shape of You"),
        ...
        ("hey sylph Teja song Dhurandhar title track can you play that on YouTube", "play Teja Dhurandhar title track"),
        ("can you open the website sylph for everyone", "open sylphforeveryone.com"),
...
Classify. Output EXACTLY ONE category name."""
    ...
    def _strip_filler(self, msg: str) -> str:
        ...
        cleaned = re.sub(
            r'^(?:hello|hi|hey|yo|hiya|howdy|ok|okay|alright)\s+(?:sylph|j\.?a\.?r\.?v\.?i\.?s\.?)\s*[,.]?\s*',
            '', cleaned, flags=re.I
        ).strip()
