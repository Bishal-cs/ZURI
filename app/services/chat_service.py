...
class ChatService:
    ...
    def process_sylph_message_stream(
        self, session_id: str, user_message: str, imgbase64: Optional[str] = None
    ) -> Iterator[Union[str, Dict[str, Any]]]:
        
        t0_sylph = time.perf_counter()
        logger.info("[SYLPH-STREAM] Session: %s | User: %.200s | img: %s",
        ...
        if category == CATEGORY_CAMERA:
            ...
            yield text
            self.save_chat_session(session_id)
            elapsed_sylph = time.perf_counter() - t0_sylph
            logger.info("[SYLPH-STREAM] Camera flow complete in %.2fs", elapsed_sylph)
            return
        ...
        if category in (CATEGORY_TASK, CATEGORY_MIXED):
            ...
            if category == CATEGORY_MIXED:
                ...
                elapsed_sylph = time.perf_counter() - t0_sylph
                logger.info("[SYLPH-STREAM] Mixed flow complete in %.2fs | tasks: %s", elapsed_sylph, task_types)
                return
            ...
            self.save_chat_session(session_id)
            elapsed_sylph = time.perf_counter() - t0_sylph
            logger.info("[SYLPH-STREAM] Task flow complete in %.2fs | tasks: %s | bg: %d", elapsed_sylph, task_types, len(bg_task_ids))
            return
        ...
        elapsed_sylph = time.perf_counter() - t0_sylph
        logger.info("[SYLPH-STREAM] %s flow complete in %.2fs | chunks: %d", route_name, elapsed_sylph, chunk_count)
